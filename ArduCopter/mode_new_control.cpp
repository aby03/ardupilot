...
float THR_ALT_P = 0.10f;
float RATE_THR_P = 0.10f;
float RATE_THR_I = 0.0f;
float RATE_THR_D = 0.0f;
float ACCEL_THR_P = 0.10f;
...
// Altitude target set
float target_z = 1000.0f; //in cm according to inbuilt code 

float curr_alt = 0.0;
float error_z = 0.0;

float _vel_target = 0.0;
float error_vel = 0.0;
...
void ModeNewControl::throttle_control()
{
    ...
    _vel_target = AC_AttitudeControl::sqrt_controller(error_z/100.0f, THR_ALT_P, accel_lim/100.0f, _dt);
	// error_z = 2000 (in cms); THR_ALT_P = 0.1; accel_lim = 250; _dt = 0.02;
    if (_vel_target < vel_min) {
        _vel_target = vel_min;
        // _limit.vel_down = true;
    }
    if (_vel_target > vel_max) {
        _vel_target = vel_max;
        // _limit.vel_up = true;
    }
...
// To limit the ki value for acceleration
 ...
 //    float throttle_out = thr_out * inverted_factor * boost_factor;
	...
	if (loop % 100 == 0){
		printf("- Alt %f ", curr_alt);
		printf(" | P %f", (error_accel*ACCEL_THR_P));
		printf(" | I %f", _vel_target);
		printf(" | D %f", accel_target);
